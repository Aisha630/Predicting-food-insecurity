...
file_handler = logging.FileHandler("district_classifier.log", mode="w")
...
def process_article(article: dict, existing_titles: set, csv_output_file: Path, idx: int, model_name:str):
    ...
    if result:
        row = {
            "title": title,
            "date": date,
            "location": ", ".join(result.location or None),
...
def main(files: list, csv_output_file: Path, model_name: str=MODEL):
    for json_input_file in files:
        with json_input_file.open('r', encoding='utf-8') as f:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(process_article, article, existing_titles, csv_output_file, idx, model_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_file", help="Output CSV file to save results", default="output.csv", required=False)
    args = parser.parse_args()

    input_files = list(Path("files").glob("*.json"))
